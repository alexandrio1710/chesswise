...
class TestRetryWithBackoff:
    def test_succeeds_immediately_with_no_errors(self):
        with patch("requests.request", return_value=MagicMock(status_code=200)) as mock_req:
    ...
    def test_retries_on_429_then_succeeds(self):
        ...
        with patch("requests.request", side_effect=responses), patch("time.sleep") as mock_sleep:
            resp = fetchers._request_with_retry("GET", "http://fake")
        assert resp.status_code == 200
    ...
    def test_gives_up_after_max_retries_on_persistent_network_error(self):
        with patch(
            "requests.request", side_effect=requests.exceptions.ConnectionError("boom")
        ), patch("time.sleep"):
    ...
    def test_gives_up_after_max_retries_on_persistent_429(self):
        with patch(
            "requests.request", return_value=MagicMock(status_code=429, headers={})
